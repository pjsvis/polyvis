...
// Setup Table
db.run("CREATE TABLE IF NOT EXISTS _stress (id INTEGER PRIMARY KEY, payload TEXT, timestamp INTEGER)");
...
const interval = setInterval(() => {
    try {
        ...
        db.run("INSERT INTO _stress (payload, timestamp) VALUES (?, ?)", [payload, Date.now()]);
