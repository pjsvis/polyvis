...
async function main() {
    ...
    if (!await Bun.file("_staging.md").exists()) {
        console.error("❌ _staging.md not found. Run 'bun scripts/harvest.ts' first.");
        process.exit(1);
    }
    ...
    for (const block of blocks) {
        ...
        if (action.toLowerCase() === 'y' || action.toLowerCase() === 'yes') {
            ...
            // 1. Definition
            const title = await question(`Title [${toTitleCase(tag)}]: `) || toTitleCase(tag);
            ...
            const scanDirs = settings.ingestion.directories;
            let filesCleaned = 0;
