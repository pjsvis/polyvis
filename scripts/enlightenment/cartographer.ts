...
const MODEL_PATH = resolve(ROOT_DIR, "experiments/enlightenment/Meta-Llama-3-8B-Instruct-Q4_K_M.gguf");
const VECTOR_PATH = resolve(ROOT_DIR, "experiments/enlightenment/enlightenment_vector_v2.gguf");
const SERVER_BIN = resolve(ROOT_DIR, "experiments/enlightenment/llama.cpp/build/bin/llama-server");
const LOG_FILE = resolve(ROOT_DIR, "experiments/enlightenment/CARTOGRAPHY_LOG_V3.md");
...
async function main() {
  ...
  await writeFile(LOG_FILE, `# The No-Sugar Log
...
`);
  ...
  for (const scale of SCALES) {
    ...
    for (const probe of PROBES) {
      ...
      const snippet = response.slice(0, 100).replace(/\n/g, " ") + (response.length > 100 ? "..." : "");
      
      let status = "✅ OK";
      if (sugar > 1) status = "🍬 SWEET";
      if (words > 100) status = "📜 VERBOSE";
      if (response.includes("fish") || response.length < 5) status = "🚨 BROKEN";
      if (sugar === 0 && words < 50 && !status.includes("BROKEN")) status = "🏆 WINNER?";

      console.log(`[Sugar: ${sugar}] ${status}`);
      
      await appendFile(LOG_FILE, `| **${scale}** | ${probe.name} | ${snippet} | ${words} | ${sugar} | ${status} |\n`);
    }
...
async function launchServer(scale: number): Promise<ChildProcess | null> {
  ...
  const server = spawn(SERVER_BIN, [
    ...
    "--control-vector-scaled", vectorArg,
    "--ctx-size", "2048",
    "--alias", "cartographer-llama"
  ]);
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => { if (!started) { server.kill(); resolve(null); } }, 20000);
  });
}

async function queryModel(prompt: string): Promise<string> {
    try {
        const res = await fetch(`http://127.0.0.1:${SERVER_PORT}/chat/completions`, {
            method: "POST",
            body: JSON.stringify({ 
    ...
    } catch { return "CONN_ERR"; }
}

function calculateSugarScore(text: string): number {
    const sugarWords = [
        "delighted", "fascinating", "pleasure", "assist", "comprehensive", 
        "landscape", "realm", "journey", "explore", "fostering", "collaborative"
    ];
    let score = 0;
    const lower = text.toLowerCase();
    sugarWords.forEach(w => { if (lower.includes(w)) score++; });
    return score;
}
