...
const nodeCount = db.query("SELECT COUNT(*) as count FROM nodes").get() as { count: number };
const edgeCount = db.query("SELECT COUNT(*) as count FROM edges").get() as { count: number };
...
const expNodes = db.query("SELECT COUNT(*) as count FROM nodes WHERE type IN ('playbook', 'debrief')").get() as { count: number };
const expEdges = db.query("SELECT COUNT(*) as count FROM edges WHERE type IN ('CITES', 'REFERENCES')").get() as { count: number };
...
if (expNodes.count > 0 && expEdges.count === 0) {
    console.warn(`⚠️  WARNING: Experience nodes exist but have NO connections to the graph.`);
}
