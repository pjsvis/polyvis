...
async function runServer() {
    ...
    if (await cssFile.exists()) { try { await Bun.spawn(["rm", cssPath]).exited; } catch {} }

    // 0.6 Ensure valid initial build
    console.log("🔨 Running initial CSS build...");
    await Bun.spawn(["bun", "run", "build:css"], { stdout: "inherit", stderr: "inherit" }).exited;

    // 1. Start Watchers
    console.log("🎨 Starting CSS Watcher...");
    const cssWatcher = Bun.spawn(["bun", "run", "watch:css"], { stdout: "inherit", stderr: "inherit" });

    console.log("📜 Starting JS Watcher...");
    const jsWatcher = Bun.spawn(["bun", "run", "watch:js"], { stdout: "inherit", stderr: "inherit" });
    ...
    const server = Bun.serve({
        ...
        async fetch(req) {
            ...
            return file.exists().then(async (exists) => {
                ...
                if (await Bun.file(htmlPath).exists()) return new Response(Bun.file(htmlPath));
                if (!extname(path)) {
                    const indexPath = join(filePath, "index.html");
                    if (await Bun.file(indexPath).exists()) return new Response(Bun.file(indexPath));
                }
