...
async function sendMCPRequest(query: string, requestId: number): Promise<{ success: boolean; error?: string; duration: number }> {
	...
	try {
		...
		if (stderr.includes("SQLITE_BUSY") || stderr.includes("disk I/O error")) {
			return { success: false, error: "Database concurrency error detected", duration };
		}

		if (stderr.includes("ERROR") || stderr.includes("Error:")) {
			return { success: false, error: `Error in stderr: ${stderr.slice(0, 200)}`, duration };
		}
		...
		try {
			...
			return { success: false, error: "No valid JSON-RPC response found", duration };
		} catch (e) {
			return { success: false, error: `Failed to parse response: ${e}`, duration };
		}
