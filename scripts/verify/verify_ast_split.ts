...
`;
  ...
  const hasFakeHeaderSplit = boxes.some(b => b.content.trim().startsWith("# Fake Header"));
  if (hasFakeHeaderSplit) {
    console.error("FAIL: BentoBoxer split on a fake header inside a code block.");
    process.exit(1);
  }

  // Check if code block is intact in the first box (or wherever it ends up)
  const codeBlockPart = "```markdown\n# Fake Header inside code block\nSome code content\n```";
  const boxWithCode = boxes.find(b => b.content.includes(codeBlockPart));

  if (!boxWithCode) {
    console.error("FAIL: Code block integrity check failed. The code block seems to have been mangled or split.");
