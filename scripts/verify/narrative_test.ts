...
const edges = db.query("SELECT source, target FROM edges WHERE type = 'SUCCEEDS'").all() as any[];
...
const genesisNodes = nodes.filter(n => !prevMap.has(n.id) && nextMap.has(n.id));

if (genesisNodes.length === 0) {
    if (nodes.length > 0 && edges.length === 0) {
        console.error("❌ Failed: Debriefs exist but no SUCCEEDS edges found. Timeline Weaver failed?");
    } else {
        console.error("❌ Failed: No clear Genesis node found (Cycle or Fragments detected).");
    }
    process.exit(1);
}

if (genesisNodes.length > 1) {
    console.warn(`⚠️  Warning: Multiple Genesis nodes found (${genesisNodes.length}). The narrative is forked.`);
}
...
timeline.forEach((node, index) => {
    ...
    const date = meta.created ? new Date(meta.created).toISOString().split('T')[0] : "Unknown Date";
    ...
    const summary = content.split('\n').find((line: string) => line.length > 50 && !line.startsWith('#')) || "No summary available.";
