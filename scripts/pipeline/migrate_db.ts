...
const run = (label: string, sql: string) => {
	try {
		...
		console.warn(
			`  ⚠️ ${label} failed (might already exist):`,
			(e as Error).message,
		);
	}
};
...
run(
	"Add domain",
	"ALTER TABLE nodes ADD COLUMN domain TEXT DEFAULT 'knowledge'",
);
run(
	"Add layer",
	"ALTER TABLE nodes ADD COLUMN layer TEXT DEFAULT 'experience'",
);
run(
	"Add order_index",
	"ALTER TABLE nodes ADD COLUMN order_index INTEGER DEFAULT 0",
);
...
run(
	"Rename definition -> content",
	"ALTER TABLE nodes RENAME COLUMN definition TO content",
);
run(
	"Rename relation -> type",
	"ALTER TABLE edges RENAME COLUMN relation TO type",
); // schema.ts uses 'type' alias for relation column? No, schema says `type: text("relation")`
