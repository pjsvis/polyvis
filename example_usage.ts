...
async function main() {
  ...
  if (!await hume.isOnline()) {
    console.error("⚠️  The Enlightenment Server is offline. Run the Golden Command.");
    return;
  }
  ...
  const logicResponse = await hume.think([
    ...
    { role: "user", content: "I walked under a ladder and then tripped. Did the ladder cause me to trip?" }
  ]);
  console.log(`Result: ${logicResponse}\n`);

  // 4. Scenario B: The "De-Fluff" Tool
  console.log("--- TEST B: DE-FLUFFING ---");
  const corporateJargon = "We need to leverage our core competencies to shift the paradigm and boil the ocean.";
