...
I have reviewed the existing briefs and integrated the new "Knowledge Package Manager" requirements.

Here is the **Amended Main Brief** (incorporating the new verbs) and the **New Registry Brief** (detailing the mechanics).

-----

### 1\. Amended Main Brief

**File:** `briefs/brief-resonance-engine.md`
...
### Summary of Updates
1.  **Main Brief:** Now includes the full lifecycle (Init $\to$ Install $\to$ Sync $\to$ Serve).
2.  **Registry Brief:** Defines *how* the tool knows what to install (Signatures) and *how* it manages updates (Lockfile).

**Next Step:**
Hand these two briefs (along with the existing Config, Graph, and MCP briefs) to the coding agent. This provides a complete, modular specification for the entire system.
```