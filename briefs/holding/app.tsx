...
import { LOCATIONS, LocationDashboardData, LocationDef } from './types';
import { fetchEnvironmentalData } from './services/weatherService';
import { fetchTideData, fetchInsight } from './services/geminiService';
import { GridCard } from './components/GridCard';
import { InsightCard } from './components/InsightCard';
import { LocationSelector } from './components/LocationSelector';
import { 
  Sun, Moon, Waves, CloudRain, Wind, 
  MapPin, RefreshCw, Thermometer, Droplets,
...
const LoadingSkeletonLine: React.FC<{ width?: string }> = ({ width = "w-full" }) => (
  <div className={`h-4 bg-slate-800/50 rounded animate-pulse ${width}`}></div>
);

const TideLevelVisual: React.FC<{ status: string; height: string }> = ({ status, height }) => {
  ...
  const colorClass = isRising ? "text-sky-400" : isFalling ? "text-indigo-400" : "text-sky-200";
    ...
    <div className="relative w-full flex-1 min-h-[160px] bg-slate-900/50 rounded-xl overflow-hidden border border-slate-700/50 mb-4 flex flex-col justify-between group">
      ...
      >
        <div className="absolute -top-5 left-0 w-[200%] h-12 overflow-hidden animate-wave">
             <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className={`w-full h-full ${colorClass} opacity-30 fill-current`}>
                 <path d="M0,0 C150,60 350,0 600,30 C850,60 1050,0 1200,30 V120 H0 V0 Z" />
             </svg>
        </div>
        <div className={`w-full h-full bg-gradient-to-t ${bgGradient} backdrop-blur-[1px]`}></div>
      </div>
      <div className="relative z-20 p-4 flex flex-col justify-between h-full items-start w-full">
         <div className="flex items-center gap-2 bg-slate-900/40 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-700/50 shadow-sm mb-auto">
            {isRising && <ArrowUp className="text-sky-400 animate-pulse" size={18} />}
            {isFalling && <ArrowDown className="text-indigo-400 animate-pulse" size={18} />}
            {!isRising && !isFalling && <Minus className="text-slate-400" size={18} />}
            <span className="text-lg font-bold text-white tracking-wide">{status}</span>
         </div>
...
const App: React.FC = () => {
  ...
  const getEmptyData = (): LocationDashboardData => ({
    weather: { temperature: "--", condition: "Loading...", windSpeed: "--", cloudCover: "--" },
    sun: { sunrise: "--:--", sunset: "--:--", goldenHourMorning: "--:--", goldenHourEvening: "--:--" },
    moon: { phase: "--", rise: "--:--", illumination: "--" },
    tides: { firstHigh: "--:--", firstLow: "--:--", secondHigh: "--:--", secondLow: "--:--", heightHigh: "--", status: "Slack" },
    insight: null,
    lastUpdated: "--:--"
  });
           ...
           if (!localStorage.getItem(key)) {
             ...
             fetchEnvironmentalData(loc).then(env => {
                 fetchTideData(loc.name).then(tides => {
                     ...
                     const data = { ...empty, ...env, tides, lastUpdated: 'Background' };
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 md:p-8 max-w-7xl mx-auto relative">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          <h1 className="text-4xl font-light tracking-tight text-white mb-2">Omni<span className="font-bold text-sky-400">View</span></h1>
        ...
        <div className="flex flex-col items-start md:items-end gap-3">
          ...
          <div className="flex items-center gap-3">
             ...
             >
                ...
                />
                {isUpdating ? "Updating..." : `Updated ${data?.lastUpdated || '--'}`}
             </button>
          </div>
        </div>
      </header>
      ...
      */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-min">
        ...
        {/* 1. CONDITIONS */}
        <GridCard title="Conditions" icon={<CloudRain size={16} />} className="lg:col-span-1">
           {!data ? <div className="animate-pulse bg-slate-800 h-24 rounded"></div> : (
             <div className="flex flex-col justify-between h-full min-h-[140px]">
               <div className="flex items-start justify-between">
                 <div>
                   <div className="text-4xl font-light text-white">{data.weather.temperature}</div>
                   <div className="text-sm text-slate-400 mt-1">{data.weather.condition}</div>
                 </div>
               ...
               <div className="grid grid-cols-2 gap-2 mt-4">
                 <div className="bg-slate-900/40 p-2 rounded border border-slate-700/30">
                    <div className="flex items-center gap-1 text-xs text-slate-500 mb-1"><Wind size={10} /> Wind</div>
                    <div className="text-sm text-slate-200">{data.weather.windSpeed}</div>
                 </div>
                 <div className="bg-slate-900/40 p-2 rounded border border-slate-700/30">
                    <div className="flex items-center gap-1 text-xs text-slate-500 mb-1"><Droplets size={10} /> Cloud</div>
                    <div className="text-sm text-slate-200">{data.weather.cloudCover}</div>
                 </div>
               </div>
             </div>
           )}
        ...
        {/* 2. SUN */}
        <GridCard title="Sun" icon={<Sun size={16} />} className="lg:col-span-1">
          {!data ? <div className="animate-pulse bg-slate-800 h-24 rounded"></div> : (
            <div className="space-y-4 flex flex-col justify-center h-full min-h-[140px]">
               <div className="flex justify-between items-center pb-3 border-b border-slate-700/50">
                 <div>
                   <div className="text-xs text-orange-400/80 mb-0.5">Sunrise</div>
                   <div className="text-xl text-white font-medium">{data.sun.sunrise}</div>
                 </div>
                 <div>
                   <div className="text-xs text-indigo-400/80 mb-0.5 text-right">Sunset</div>
                   <div className="text-xl text-white font-medium text-right">{data.sun.sunset}</div>
                 </div>
               </div>
               <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>
                    <span className="text-xs text-slate-400 uppercase">Golden Hour</span>
                  </div>
        ...
        {/* 3. MOON */}
        <GridCard title="Moon" icon={<Moon size={16} />} className="lg:col-span-1">
           {!data ? <div className="animate-pulse bg-slate-800 h-24 rounded"></div> : (
             <div className="flex flex-col justify-between h-full gap-2 min-h-[140px]">
               <div className="flex items-center gap-4 mt-2">
                  ...
                  <div>
                    <div className="text-base text-white font-medium leading-tight mb-1">{data.moon.phase}</div>
                    <div className="text-xs text-slate-400">{data.moon.illumination} Illum.</div>
                  </div>
               </div>
        ...
        <GridCard title="Tide Schedule" icon={<Waves size={16} />} className="lg:col-span-1 lg:row-span-2 bg-blue-950/20">
          {!data ? <div className="animate-pulse h-full bg-slate-800 rounded"></div> : (
            <div className="flex flex-col h-full">
              <TideLevelVisual status={data.tides.status} height={data.tides.heightHigh} />
              <div className="space-y-4 relative pl-2 pb-2 mt-2">
                ...
                <div className="relative pl-8 group">
                  ...
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs text-sky-400 font-semibold">High Tide</span>
                    <span className="text-lg text-white font-mono">{data.tides.firstHigh}</span>
                  </div>
                </div>
                ...
                <div className="relative pl-8 group opacity-80">
                  ...
                  <div className="flex justify-between items-baseline">
                    ...
                    <span className="text-base text-slate-300 font-mono">{data.tides.firstLow}</span>
                  </div>
                </div>
                ...
                {data.tides.secondHigh && data.tides.secondHigh !== "--:--" && (
                   <div className="relative pl-8 group">
                    ...
                    <div className="flex justify-between items-baseline">
                      ...
                      <span className="text-base text-white/90 font-mono">{data.tides.secondHigh}</span>
                    </div>
                  </div>
                )}
              </div>
             ...
             <div className="flex-1 p-6 flex flex-col justify-center gap-4">
               ...
               {insightLoading && (
                 <div className="absolute top-6 right-6 flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase tracking-widest text-slate-500">AI Processing</span>
                 </div>
               )}
             </div>
          ) : (
      ...
      <footer className="mt-12 text-center text-slate-600 text-xs">
        <p>OmniView © {new Date().getFullYear()}. Weather data via Open-Meteo. Tide & Insight via Gemini.</p>
