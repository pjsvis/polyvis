...
async function analyzeText() {
  const text = "Apple Inc. is looking at buying U.K. startup for $1 billion. Tim Cook announced this in San Francisco.";
  ...
  nerResults.forEach((entity: any) => {
    console.log(`${entity.word.padEnd(20)} ${entity.entity.padEnd(15)} Score: ${entity.score.toFixed(3)}`);
  });
  ...
  const classificationResult = await classifier(text, ['business', 'technology', 'politics', 'sports']);
