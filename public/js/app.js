...
var sigma_explorer_default = () => ({
	...
	loadGraph(db) {
		...
		this.renderer.on("clickStage", () => {
			console.log(
				`[SigmaDebug] clickStage. Blocked? ${this.clickBlock}. Hovered? ${this.hoveredNode}`,
			);
			if (this.clickBlock) {
			...
			if (this.hoveredNode) {
				console.log(
					`[SigmaDebug] clickStage FALLBACK to hoveredNode: ${this.hoveredNode}`,
				);
		...
		container.addEventListener(
			...
			(e) => {
				if (this.hoveredNode) {
					console.log(
						`[SigmaDebug] Mousedown on Node ${this.hoveredNode} -> STOPPING PROPAGATION`,
					);
					e.stopPropagation();
				}
			},
	...
	toggleColorViz(type, force = false) {
		...
		if (type === "louvain") {
			...
			const counts = {};
			Object.values(communities).forEach(
...
var graph_default = () => ({
	...
	savePNG() {
		...
		img.src =
			"data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(data2)));
	},
});
...
var Ge = k(/\\(punct)/, "gu")
	.replace(/punct/g, D)
	.getRegex();
var Ne = k(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/)
...
`,
						...
						.replace(this.rules.other.listReplaceTabs, (O) =>
							" ".repeat(3 * O.length),
						),
					h = e.split(
						`
...
`,
						...
						if (
							...
							Y.test(h) || ee.test(h) || fe.test(h) || O.test(h) || V.test(h))
						)
							break;
