...
function dotProduct(a, b2) {
	...
	if (typeof b2 === "string") vecB = JSON.parse(b2);
		...
		try {
			...
			neighbors.forEach(([targetId, score]) => {
				if (this.graph.hasNode(targetId)) {
					if (
						!this.graph.hasEdge(nodeId, targetId) &&
						!this.graph.hasEdge(targetId, nodeId)
					) {
						const edgeId = this.graph.addEdge(nodeId, targetId, {
							...
							color:
								getComputedStyle(document.documentElement)
									.getPropertyValue("--color-ghost-edge")
									.trim() || "#FFD700",
							ghost: true,
						});
		...
		if (type === "louvain") {
			...
			if (!this.louvainCommunities) {
				...
				while (attempts < maxAttempts) {
					...
					if (uniqueCount >= 3 && uniqueCount <= 7) break;
					if (uniqueCount > 7) {
				...
				const sortedIds = Object.keys(counts2).sort(
					(a, b2) => counts2[b2] - counts2[a],
				);
