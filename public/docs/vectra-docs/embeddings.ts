...
export class EmbeddingsClient {
  ...
  constructor(opts?: {
    ...
    this.openaiKey =
      opts?.openaiKey ?? process.env.OPENAI_API_KEY ?? null;
    this.openaiModel = opts?.openaiModel ?? process.env.OPENAI_EMBEDDING_MODEL ?? "text-embedding-3-small";
    this.ollamaBaseUrl = opts?.ollamaBaseUrl ?? process.env.OLLAMA_BASE_URL ?? null;
   ...
   */
  static dot(a: FloatVector | null | undefined, b: FloatVector | null | undefined): number {
    if (!a || !b) return 0;
   ...
   */
  static cosineSimilarity(a: FloatVector | null | undefined, b: FloatVector | null | undefined): number | null {
   ...
   */
  async embedWithOpenAI(texts: string[], options?: { model?: string, timeoutMs?: number }): Promise<FloatVector[]> {
    if (!Array.isArray(texts) || texts.length === 0) {
      throw new EmbeddingsError("embedWithOpenAI requires a non-empty array of texts");
    }
    const key = this.openaiKey;
    if (!key) {
      throw new EmbeddingsError("OpenAI API key not configured (OPENAI_API_KEY)");
    }
    ...
    if (!resp.ok) {
      ...
      throw new EmbeddingsError(`OpenAI embeddings request failed: ${resp.status} ${resp.statusText} - ${txt}`);
    }

    const json = await resp.json().catch(() => null);
    if (!json || !Array.isArray(json.data)) {
      throw new EmbeddingsError("Unexpected OpenAI response shape for embeddings");
    }
  ...
  async rankWithOpenAI(
    query: string,
    candidates: Candidate[],
    opts?: { topK?: number; useExistingCandidateVectors?: boolean; model?: string }
  ): Promise<Candidate[]> {
    if (!query || typeof query !== "string") throw new EmbeddingsError("rankWithOpenAI requires a query string");
  ...
  async ollamaRerank(
    query: string,
    candidates: Candidate[],
    opts?: { ollamaModel?: string; topK?: number; maxPromptItems?: number; baseUrl?: string }
  ): Promise<Candidate[]> {
    const baseUrl = opts?.baseUrl ?? this.ollamaBaseUrl;
    if (!baseUrl) {
      throw new EmbeddingsError("OLLAMA_BASE_URL not configured; cannot use Ollama reranker");
    }
    ...
    const prompt = [system, `Query: ${query}`, `Candidates:\n${promptItems.join("\n")}`, "Return an array of {id, score}."].join("\n\n");
    ...
    if (!resp.ok) {
      const txt = await resp.text().catch(() => "<no body>");
      throw new EmbeddingsError(`Ollama request failed: ${resp.status} ${resp.statusText} - ${txt}`);
    }
    ...
    if (firstIdx === -1 || lastIdx === -1 || lastIdx <= firstIdx) {
      ...
      try {
        ...
        if (Array.isArray(parsedWhole)) {
          return this._applyOllamaScoresToCandidates(parsedWhole, candidates, topK);
        }
      } catch {
        throw new EmbeddingsError("Failed to parse Ollama rerank output as JSON array");
      }
    }
    ...
    try {
      ...
      throw new EmbeddingsError("Failed to parse JSON substring from Ollama response");
    }
  ...
  private _applyOllamaScoresToCandidates(parsedArray: Array<{ id: string; score: number }>, candidates: Candidate[], topK: number): Candidate[] {
    // Build score map
  ...
  async rerankWithHybrid(
    query: string,
    candidates: Candidate[],
    opts?: { topK?: number; forceOllama?: boolean; openaiModel?: string; ollamaModel?: string }
  ): Promise<Candidate[]> {
    const topK = opts?.topK ?? this.defaultTopK;

    // If forced to use Ollama
    if (opts?.forceOllama) {
      if (!this.ollamaBaseUrl) throw new EmbeddingsError("Ollama base URL not configured");
      return this.ollamaRerank(query, candidates, { topK, ollamaModel: opts?.ollamaModel });
    }

    // Prefer OpenAI if key available
    if (this.openaiKey) {
      // If candidates already have vectors, compute query embedding and rank
      const candidatesHaveVectors = candidates.every((c) => Array.isArray(c.vector) && c.vector.length > 0);
      try {
        if (candidatesHaveVectors) {
          const qvecs = await this.embedWithOpenAI([query], { model: opts?.openaiModel });
          const qvec = qvecs[0];
          if (!qvec || qvec.length === 0) throw new EmbeddingsError("Failed to get query embedding");
          return this.rankCandidatesByEmbedding(qvec, candidates, topK);
        } else {
          // Attempt to batch-embed candidate texts if present
          const textsToEmbed = candidates.map((c) => c.text ?? c.metadata?.excerpt ?? "");
          const candidateVecs = await this.embedWithOpenAI(textsToEmbed, { model: opts?.openaiModel });
    ...
    throw new EmbeddingsError("No embedding provider configured (OPENAI_API_KEY or OLLAMA_BASE_URL required)");
  }
}
