...
export class ZombieDefense {
    ...
    static async scan(excludePids: string[] = []): Promise<ZombieReport> {
        ...
        try {
            ...
            processes.forEach(p => {
                ...
                const isWhitelisted = ZombieDefense.WHITELIST.some(w => p.includes(w));
                ...
                if (isWhitelisted) {
                    ...
                    if (!p.includes("detect_zombies.ts") && !p.includes("ZombieDefense")) {
        ...
        if (report.ghosts.length > 0 || report.duplicates.length > 0 || report.unknowns.length > 0) {
            report.clean = false;
        }
    ...
    static async killZombies(report: ZombieReport) {
        let targets = [...new Set([...ZombieDefense.extractPids(report.ghosts), ...ZombieDefense.extractPids(report.duplicates)])];
        ...
        console.error(`🔪 Killing ${targets.length} zombie processes: ${targets.join(", ")}`);
        try {
    ...
    static async assertClean(serviceName: string, interactive = false) {
        ...
        if (interactive) {
             const targets = ZombieDefense.extractPids([...report.ghosts, ...report.duplicates]);
             if (targets.length > 0) {
