...
export class DatabaseFactory {
     ...
     */
    static connect(path: string, options: { readonly?: boolean; create?: boolean } = {}): Database {
        ...
        const currentMode = db.query("PRAGMA journal_mode;").get() as { journal_mode: string };
    ...
    static performHealthCheck(db: Database) {
        ...
        if (journal.journal_mode !== 'wal') console.warn("⚠️ HealthWarning: WAL mode not active.");
        if (mmap.mmap_size !== 0) console.warn("⚠️ HealthWarning: mmap should be 0 for stability.");
        if (busy.timeout < 5000) console.warn("⚠️ HealthWarning: busy_timeout < 5000ms.");
