...
export class EnlightenedProvider {
  ...
  async think(messages: Message[]): Promise<string> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Enlightenment Error: ${response.status} ${response.statusText}`);
      }
  ...
  async defluff(inputText: string): Promise<string> {
    return this.think([
      { role: "system", content: "You are a ruthless editor. Rewrite the following text to be concise, factual, and free of corporate buzzwords. Return ONLY the rewritten text." },
